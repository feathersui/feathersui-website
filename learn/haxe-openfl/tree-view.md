...
Set the tree view's [`layout`](https://api.feathersui.com/current/feathers/controls/TreeView.html#layout) property to change how its children are positioned and sized. By default, a tree view uses [`VerticalListLayout`](./vertical-list-layout.md), but it may be changed to a different layout, if desired.
