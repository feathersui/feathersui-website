...
Set the group list view's [`layout`](https://api.feathersui.com/current/feathers/controls/GroupListView.html#layout) property to change how its children are positioned and sized. By default, a group list view uses [`VerticalListLayout`](./vertical-list-layout.md), but it may be changed to a different layout, if desired.
